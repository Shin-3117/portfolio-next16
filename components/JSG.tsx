  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      if (canvasRef.current) {
        ...
        // 플레이어 위치를 새로운 중앙으로 조정
        if (player && oldWidth > 0 && oldHeight > 0) {
          player.x = canvasRef.current.width / 2;
          player.y = canvasRef.current.height / 2;
        }

        // 게임 상태에 관계없이 현재 프레임 다시 그리기
        if (canvasRef.current) {
          const canvas = canvasRef.current;
          const c = canvas.getContext("2d");
          if (c) {
            ...
            enemies.forEach(enemy => enemy.draw(c));
            particles.forEach(particle => particle.draw(c));
            superBullets.forEach(superBullet => superBullet.draw(c));
          }
        }
      }
    };
